...
const MenuItem = ({id, imgUrl ,name ,bread , ingredients}) => {
  ...
  return (
    ...
    <div id={id} className='relative max-w-[450px] h-[450px] w-full mx-auto my-3 py-16 px-4 ' >
        ...
        <Modal open={openModal} name={name} imgUrl={imgUrl} bread={bread} ingredients={ingredients} onClose={() => setOpenModal(false)}/>
        <div className='bg-white/90 rounded-lg my-1' >
            ...
            <div>
              <button className='block mx-auto' onClick={() => {setOpenModal(!openModal)}}>Modal Button</button>
            </div>
