...
const Navbar = () => {
  ...
  return (
    <div id='navbar' className='flex justify-between items-center h-24 max-w-[1240px] mx-auto px-4 text-white absolute'>
        ...
        <div onClick={handleNav} className='flex flex-end text-[#00df9a] top-2 right-2 z-[999] fixed hover:scale-[120%] ease-in duration-200 cursor-pointer '  >
        ...
        <div className={nav ? 'fixed left-0 top-0 w-[80%] h-full border-r border-r-gray-900 bg-[#000300]/60 ease-in-out duration-500 z-[999] md:w-[35%]' : 'fixed left-[-100%] z-[999]'} >
            ...
            <div className='flex flex-col'>
                ...
                {dataArray.map((item, index) => (
                    <a onClick={handleNav} className='text-[#00df9a] flex justify-start hover:scale-[120%] ease-in duration-200' key={index} href={`#${item.id}`}>{item.name}</a>
                ))}
                </div>
                <a onClick={handleNav} href="#home" className='text-[#00df9a] mx-auto mt-4 hover:scale-[120%] ease-in duration-200'>Home</a>
                <a onClick={handleNav} href="#contact" className='text-[#00df9a] mx-auto hover:scale-[120%] ease-in duration-200'>Contact</a>
            </div>
        </div>
    </div>
  )
